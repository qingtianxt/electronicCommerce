				...
				<table class="table table-striped">
					...
					<c:forEach items="${userBeans }" var="item" varStatus="status">
						<tr>
							...
							<td><a
								href="${pageContext.request.contextPath}/admin/userServlet?method=search&search=${item.username }">${item.username }</a></td>
							<td>${item.nickname }</td>
							<td>${item.truename }</td>
							...
							<td><c:if test="${item.status==1 }">
									<a
										href="${pageContext.request.contextPath}/admin/userServlet?method=update&status=0&id=${item.id }">冻结</a>
								</c:if> <c:if test="${item.status==0 }">
									<a
										href="${pageContext.request.contextPath}/admin/userServlet?method=update&status=1&id=${item.id }">解冻</a>
								</c:if></td>
						</tr>
					</c:forEach>
				</table>
