...
<body>
		...
		<c:forEach items="${productBeans}" var="item">
			<div class="product">
				<a href="productShowServlet?method=info&id=${item.id }">
					<!-- 点击图片或者点击商品名称都会跳到servlet 类型是info -->
	...
	${param.status==2?"<div class='alert alert-danger' role='alert'>提交成功</div>":"" }
	${param.status==3?"<div class='alert alert-danger' role='alert'>提交失败</div>":"" }
</body>
</html>