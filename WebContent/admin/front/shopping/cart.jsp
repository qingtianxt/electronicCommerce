...
<body>
	...
	<form action="${pageContext.request.contextPath}/admin/front/shopping/shoppingServlet?method=makeOrder"
		method="post">
		<div>
			<c:forEach items="${productOrderBeans}" var="pItem">
				<div class="aForm" name="aForm">
					...
					<ul class="aForm0">
						<li>原价： <span type="text" name="oPrice" class="oPrice">${pItem.productBean.originalPrice*pItem.number}</span>
							元
						</li>
					...
					<ul class="aForm0">
						...
						<li><a
							href="${pageContext.request.contextPath}/admin/front/shopping/shoppingServlet?method=del&productId=${pItem.productBean.id}"><button class="btn btn-danger" type="button">删除商品
								</button>
						</a></li>
					</ul>
				</div>
	...
	${param.status==1?"<div class='alert alert-danger' role='alert'>购物车空空如也</div>":"" }
</body>
</html>